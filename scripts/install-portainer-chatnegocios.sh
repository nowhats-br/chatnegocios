...
# Rede interna para banco e serviços auxiliares
...
# Portainer em overlay attachable (pode gerenciar Swarm)
docker run -d \
...
docker stack deploy -c <(sed -E "s/app_net/${APP_NET_SWARM}/g; s/^[[:space:]]*ports:[[:space:]]*$//; s/^[[:space:]]*-[[:space:]]*\"5432:5432\"[[:space:]]*$//" "$PROJECT_DIR/scripts/postgres-compose.yml") chatdb

# ChatNegocios como stack (usando redes overlay)
...
export DATABASE_URL="postgresql://postgres:postgres@postgres:5432/chatnegocios"
export CORS_ORIGINS="https://${CHATNEGOCIOS_DOMAIN}"

docker stack deploy -c <(sed -E "s/app_net/${APP_NET_SWARM}/g; s/^[[:space:]]*ports:[[:space:]]*$//; s/^[[:space:]]*-[[:space:]]*\"3001:3001\"[[:space:]]*$//; s/^[[:space:]]*-[[:space:]]*\"8081:80\"[[:space:]]*$//" "$PROJECT_DIR/scripts/chatnegocios-compose.yml") chatnegocios
